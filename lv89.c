...
static int32_t wf_step(wf_tb_t *tb, int32_t is_ext, int32_t tl, const char *ts, int32_t ql, const char *qs, int32_t n, wf_diag_t *a, int32_t *t_end, int32_t *q_end)
{
...
uint32_t *lv_ed_unified(int32_t tl, const char *ts, int32_t ql, const char *qs, int32_t is_ext, int32_t *score, int32_t *t_endl, int32_t *q_endl, int32_t *n_cigar)
{
	...
	a[0].d = 0, a[0].k = -1;
	while (1) {
		n = wf_step(n_cigar? &tb : 0, is_ext, tl, ts, ql, qs, n, a, &t_end, &q_end);
		if (n < 0) break;
		++s;
	}
	free(a);
